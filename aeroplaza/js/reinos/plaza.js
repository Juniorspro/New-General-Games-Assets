...
/* el barrio: tres casas de Rezona mirando a la plaza (la del medio es la tuya) */
const CASAS = [[-13, 43], [2, 51], [17, 43]];
/* los hoteles del otro lado del agua, cada uno en su islote: [x, z] */
const HOTELES = [[-50, -104], [30, -114], [98, -60]];
...
export function alturaPlaza(x, z) {
  ...
  /* los islotes de los hoteles */
  for (const [hx, hz] of HOTELES) { const d = Math.hypot(x - hx, z - hz); if (d < 22) h = Math.max(h, 1.1 + (-6 - 1.1) * suaveEntre(12, 21, d)); }
  /* el lago: un cuenco */
...
const CAMINOS = [[ESTACION, PLAZA], [PLAZA, TIENDA], [PLAZA, [10, -2]], [PLAZA, [-10, -30]], [PLAZA, PROBADOR], [PLAZA, BARRIO]];
...
const hayPasto = (x, z) => { const h = alturaPlaza(x, z); return h > 1.2 && enCamino(x, z) > 2 && Math.hypot(x - PLAZA[0], z - PLAZA[1]) > 10.5 && Math.hypot(x - ESTACION[0], z - ESTACION[1]) > 7.5 && Math.hypot(x - TIENDA[0], z - TIENDA[1]) > 8.5 && !CASAS.some(([cx, cz]) => Math.hypot(x - cx, z - cz) < 6.5); };
...
export function crearPlaza(ctx) {
  ...
  for (let i = 0; i < 140 && arb.length < 46; i++) {
    ...
    if (!hayPasto(x, z) || Math.hypot(x - LAGO[0], z - LAGO[1]) < 20 || Math.hypot(x - HUERTA[0], z - HUERTA[1]) < 9 || Math.hypot(x - BARRIO[0], z - BARRIO[1]) < 18 || enCamino(x, z) < 3.5) continue;
  ...
  for (let i = 0; i < 30; i++) { const a = r() * 6.28, d = 12 + r() * 60, x = Math.cos(a) * d, z = Math.sin(a) * d; if (enCamino(x, z) < 2.5 || Math.hypot(x - PLAZA[0], z - PLAZA[1]) < 12 || Math.hypot(x - BARRIO[0], z - BARRIO[1]) < 17) continue; pie.push([x, z, 0.4 + r() * 1.2]); }
  ...
  /* el barrio: tres casas redondas de Rezona; en la del medio se entra a la tuya */
  let puertaCasa = null;
  CASAS.forEach(([x, z], i) => {
  ...
  /* medusas de gelatina alrededor del globo del lago */
  ...
  return {
    ...
    actualizar(dt, jp, cielo) {
      ...
      for (const f of faroles) { const L = f.userData.luz; L.material.emissiveIntensity = 0.3 + noche * 3.2; L.material.opacity = Math.min(0.95, L.userData.op0 + noche * 0.55); }
