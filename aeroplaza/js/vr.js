...
export class VR {
  ...
  dibujar(motor, dt, real = dt, encima = null) {
    ...
    if (F.t >= 0.5) {
      ...
      const manos = this.datosManos?.() || '';
      if (this.verFps && this.el) for (const s of this.el.querySelectorAll('.vr-fps')) s.textContent = `${Math.round(F.valor)} fps · ${t('vr_mundo')} ${Math.round(F.valorMundo)}${manos ? ' · ' + manos : ''}`;
    }
  }
}
