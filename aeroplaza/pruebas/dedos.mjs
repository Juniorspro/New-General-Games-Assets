...
/* el celu parado dos veces: sin girar (?giro=no) y girado 90° (lo de siempre ahora).
   Los puntos se piensan en coordenadas del JUEGO y se pasan a la pantalla física */
const errores = [];
for (const modo of ['no', 'normal', 'reves']) {
const { pag, ctx, errores: E } = await abrir(nav, `directo&pausa&calidad=baja&giro=${modo}`, { ancho: 390, alto: 844, movil: true });
errores.push(...E);
await pag.waitForFunction(() => window.__A && window.__A.reino && document.querySelector('.hud'), null, { timeout: 120000, polling: 250 });
await avanzar(pag, 5);
const P = await pag.evaluate(() => ({ g: window.__A.Pantalla.girado, inv: window.__A.Pantalla.invertido, w: window.__A.Pantalla.w, h: window.__A.Pantalla.h }));
const W = 390, H = 844;
const aFis = ([x, y]) => !P.g ? [x, y] : P.inv ? [y, H - x] : [W - y, x];
const aJue = ([x, y]) => !P.g ? [x, y] : P.inv ? [H - y, x] : [y, W - x];
const et = `[${modo}] `;
prueba(et + (modo === 'no' ? 'sin girar' : 'girado 90°') + ` · el juego mide ${P.w}×${P.h}`, modo === 'no' ? (!P.g && P.w === 390) : (P.g && P.w === 844 && P.h === 390 && P.inv === (modo === 'reves')));
...
const [px, py] = await centro('.dedo-palanca'), [gpx, gpy] = aJue([px, py]);
prueba(et + 'el dedo en la palanca llega a la capa de dedos', ['palanca', 'dedos'].includes(await quien(px, py)), await quien(px, py));
const antes = await pag.evaluate(() => window.__A.yo.p.clone());
await toque('touchStart', [[px, py]]);
await toque('touchMove', [aFis([gpx, gpy - 30])]); await toque('touchMove', [aFis([gpx, gpy - 60])]);
...
prueba(et + 'la palanca mueve al muñeco', d > 2, `${d.toFixed(1)} m · ${durante}`);
/* saltar */
const [sx, sy] = await centro('[data-b=salta]');
prueba(et + 'el botón ⤒ no está tapado', (await quien(sx, sy)) === 'salta', await quien(sx, sy));
...
prueba(et + '⤒ salta', salto.e === 'salta' || salto.vy > 1, `${salto.e} vy ${salto.vy.toFixed(1)}`);
/* la cámara con el dedo derecho */
const yaw0 = await pag.evaluate(() => window.__A.cam.yaw);
const cx = P.w * 0.77, cy = P.h * 0.36;
await toque('touchStart', [aFis([cx, cy])]); await toque('touchMove', [aFis([cx - 50, cy])]); await avanzar(pag, 2); await toque('touchMove', [aFis([cx - 100, cy + 10])]); await avanzar(pag, 2); await toque('touchEnd', []);
const yaw1 = await pag.evaluate(() => window.__A.cam.yaw);
prueba(et + 'arrastrar a la derecha gira la cámara', Math.abs(yaw1 - yaw0) > 0.3, `${(yaw1 - yaw0).toFixed(2)} rad`);
/* dos dedos a la vez: caminar y girar */
await toque('touchStart', [[px, py], aFis([cx, cy + 60])]); await toque('touchMove', [aFis([gpx + 50, gpy]), aFis([cx - 40, cy + 60])]); await avanzar(pag, 20); await toque('touchEnd', []);
prueba(et + 'dos dedos a la vez (palanca + cámara)', true);
...
prueba(et + 'el botón ✋ no está tapado', (await quien(ax, ay)) === 'accion', await quien(ax, ay));
await toque('touchStart', [[ax, ay]]); await avanzar(pag, 3); await toque('touchEnd', []); await avanzar(pag, 3);
prueba(et + '✋ habla con Nimbo', await pag.locator('.dialogo').count() === 1);
await ctx.close();
}
