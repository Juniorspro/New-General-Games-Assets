#!/usr/bin/env python3
"""Garfio en un solo archivo HTML, para abrir con doble clic.
...
consola. Y nueve archivos sueltos no se mandan por mail.
...
LOS BINARIOS VAN EN BASE64, y son seis: uno del juego y cinco del vestido. Base64
infla un 37%, así que cada kilobyte que se ahorra preparando vale 1,37 acá — por
eso el arte se guarda al tamaño en que se muestra y no a 1024.
...
def main():
    ...
    css = (AQUI / "css" / "g.css").read_text(encoding="utf-8")
    ...
    html = html.replace('<link rel="stylesheet" href="css/g.css">',
                        f"<style>\n{css}\n</style>")
    ...
    html = html.replace('<script type="module" src="js/main.js"></script>',
                        "<script>\n(() => {\n" + "\n".join(partes) + "\n})();\n</script>")
    destino = AQUI / "garfio-en-un-archivo.html"
