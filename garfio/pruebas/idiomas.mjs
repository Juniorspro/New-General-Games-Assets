...
const url = "file://" + path.resolve("garfio-en-un-archivo.html");
...
const marca = /(?:^|\s)(?:doc|idioma|menu|como|hud|fin|piso)\.[a-z0-9-]+(?:\s|$)/i;
for (const cod of ["pt", "es", "en"]) {
  ...
  // El final: se lo tira abajo de la cámara para poder leer la lista.
  await pg.click("#m-jugar"); await pg.waitForTimeout(300);
  await pg.evaluate(() => {
    const p = window.GARFIO.partida;
    p.ancla = null; p.y = p.cam + 3000; p.vy = 20; p.alto = 400;
  });
