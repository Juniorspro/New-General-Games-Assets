...
for (const sel of ["#mando-saltar", "#mando-volver"]) {
...
// No pueden taparse entre ellos ni caer fuera de la pantalla.
const sitio = await pg.evaluate(() => {
  const a = document.querySelector("#mando-volver").getBoundingClientRect();
  const b = document.querySelector("#mando-saltar").getBoundingClientRect();
  return { separados: a.right < b.left, dentro: a.left >= 0 && b.right <= innerWidth && b.bottom <= innerHeight };
});
ch("los dos entran en la pantalla y no se pisan", sitio.separados && sitio.dentro);
...
// Darse vuelta.
const antes = await pg.evaluate(() => window.PIQUE.partida.j.dir);
await pg.tap("#mando-volver");
await pg.waitForTimeout(160);
const despues = await pg.evaluate(() => ({ dir: window.PIQUE.partida.j.dir,
                                           giro: window.PIQUE.partida.giroT }));
ch("el boton de darse vuelta da vuelta la carrera", despues.dir === -antes,
   `${antes} → ${despues.dir}`);
ch("y dispara la animacion de pivote", despues.giro > 0, `${despues.giro} cuadros`);
