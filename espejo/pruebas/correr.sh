...
python3 -m http.server 8806 --bind 127.0.0.1 >/dev/null 2>&1 &
SERV=$!
...
echo
echo "── los cuarenta niveles, resueltos de vuelta ─────────────────"
node pruebas/niveles.mjs
echo
echo "── el teléfono: que el toque caiga en la celda justa ─────────"
