#!/usr/bin/env python3
"""Arma campo-en-un-archivo.html: el juego entero en un solo HTML.

    python3 empaquetar.py

Que resuelve. El juego son nueve modulos de JavaScript, three.js, el cargador
de GLB, el modelo del perro, el cielo 360 y el banner del menu. Nada de eso se
...
# EL ORDEN IMPORTA: cada modulo se envuelve en una funcion que corre al
# definirse, asi que el que use a otro tiene que ir DESPUES. `terreno` antes que
# `pasto` y que `perro` porque los dos le piden la altura del suelo.
ORDEN = ["assets", "mundo", "terreno", "pasto", "escena", "camara", "control",
         "audio", "perro"]
ENTRADA = "main"
VENDOR = [("three", "vendor/three.module.min.js"),
          ("bgu", "vendor/BufferGeometryUtils.js"),
          ("gltf", "vendor/GLTFLoader.js")]
VENDOR_ALIAS = {"./three.module.min.js": "three", "./BufferGeometryUtils.js": "bgu",
                "./GLTFLoader.js": "gltf",
                "../vendor/three.module.min.js": "three", "../vendor/GLTFLoader.js": "gltf"}
...
def main():
    ...
    # --- los binarios ------------------------------------------------------
    mapa, crudo = {}, 0
    # Los .mp3 entran si estan. Hoy no estan: el generador de audio de Rezona
    # estuvo caido toda la sesion y el juego suena con la capa sintetizada, que
    # es justo el caso para el que esa capa existe. El dia que aparezcan, se
    # copian a assets/ y este empaquetador los mete sin tocar una linea.
    for patron in ("*.glb", "*.webp", "*.mp3"):
        for p in sorted((AQUI / "assets").glob(patron)):
            mapa[f"assets/{p.name}"] = data_uri(p); crudo += p.stat().st_size
    # `globalThis.ARCHIVOS` y no una funcion `registrar`: `assets.js` lo lee de
    # ahi, asi que el codigo del juego es EL MISMO suelto que empaquetado.
