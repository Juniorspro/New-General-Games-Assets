#!/usr/bin/env python3
"""Arma pique3d-en-un-archivo.html: el juego 3D entero en un solo HTML.
...
ORDEN = ["assets", "mundo", "azar", "guardado", "audio", "fisica", "piezas",
         "entidades", "escena", "actores", "generador", "juego", "interfaz"]
ENTRADA = "main"
...
def main():
    ...
    # --- los binarios ------------------------------------------------------
    mapa, crudo = {}, 0
    for p in sorted((AQUI / "assets" / "glb").glob("*.glb")):
        mapa[f"assets/glb/{p.name}"] = data_uri(p); crudo += p.stat().st_size
    for p in sorted((AQUI / "assets" / "tex").glob("*.webp")):
        mapa[f"assets/tex/{p.name}"] = data_uri(p); crudo += p.stat().st_size
    if con_musica:
    ...
    destino = AQUI / "pique3d-en-un-archivo.html"
