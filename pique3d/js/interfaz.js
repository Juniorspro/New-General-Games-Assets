...
// --- HUD -----------------------------------------------------------------
// --- pantalla completa ---------------------------------------------------
// En el telefono, la barra del navegador se come una franja y en horizontal se
// come casi un tercio. Pantalla completa no es un lujo: es la diferencia entre
// ver lo que viene y no verlo.
export async function pantallaCompleta() {
  ...
  } catch (err) { /* iOS en Safari no lo permite y no hay nada que hacer */ }
  try { await screen.orientation?.lock?.("landscape"); } catch (err) { /* idem */ }
}
